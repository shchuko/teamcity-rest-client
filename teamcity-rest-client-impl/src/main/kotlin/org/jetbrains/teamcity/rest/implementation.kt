...
internal fun createGuestAuthCoroutinesInstance(serverUrl: String): TeamCityCoroutinesInstanceImpl {
    return TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/guestAuth/", null, false)
}

internal fun createHttpAuthCoroutinesInstance(
    serverUrl: String,
    username: String,
    password: String
): TeamCityCoroutinesInstanceImpl {
    val authorization = Base64.getEncoder().encodeToString("$username:$password".toByteArray())
    return TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/httpAuth/", "Basic $authorization", false)
}

internal fun createTokenAuthCoroutinesInstance(serverUrl: String, token: String): TeamCityCoroutinesInstanceImpl {
    return TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/", "Bearer $token", false)
}
...
) : TeamCityCoroutinesInstance {
    override fun withLogResponses() = TeamCityCoroutinesInstanceImpl(serverUrl, serverUrlBase, authHeader, true)
    override fun withTimeout(timeout: Long, unit: TimeUnit) =
        TeamCityCoroutinesInstanceImpl(serverUrl, serverUrlBase, authHeader, true, timeout, unit)
    ...
    override fun all(): Flow<BuildAgent> {
        ...
        return if (locator.isNotEmpty()) {
            lazyPagingFlow(instance,
                ...
                convertToPage = { Page(data = it.agent.map { BuildAgentImpl(it, false, instance) }, it.nextHref) })
        } else {
...
private class VcsRootLocatorImpl(private val instance: TeamCityCoroutinesInstanceImpl) : VcsRootLocatorEx {
    override fun all(): Flow<VcsRoot> = lazyPagingFlow(instance,
        ...
        convertToPage = { Page(it.`vcs-root`.map { VcsRootImpl(it, false, instance) }, it.nextHref) })
