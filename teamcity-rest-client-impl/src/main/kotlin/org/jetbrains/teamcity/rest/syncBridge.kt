...
internal fun createGuestAuthInstance(serverUrl: String): TeamCityInstance {
    val delegate = TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/guestAuth/", null, false)
    return TeamCityInstanceBridge(delegate)
}

internal fun createHttpAuthInstance(serverUrl: String, username: String, password: String): TeamCityInstance {
    val authorization = Base64.getEncoder().encodeToString("$username:$password".toByteArray())
    val delegate = TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/httpAuth/", "Basic $authorization", false)
    return TeamCityInstanceBridge(delegate)
}

internal fun createTokenAuthInstance(serverUrl: String, token: String): TeamCityInstance {
    val delegate = TeamCityCoroutinesInstanceImpl(serverUrl.trimEnd('/'), "/", "Bearer $token", false)
    return TeamCityInstanceBridge(delegate)
}

private fun <T> lazyBlocking(block: suspend () -> T): Lazy<T> = lazy { runBlocking { block() } }

private class TeamCityInstanceBridge(
    private val delegate: TeamCityCoroutinesInstance
) : TeamCityInstance() {
    override val serverUrl: String by lazy { delegate.serverUrl }

    override fun withLogResponses(): TeamCityInstance {
        return TeamCityInstanceBridge(delegate.withLogResponses())
    }

    override fun withTimeout(timeout: Long, unit: TimeUnit): TeamCityInstance {
        return TeamCityInstanceBridge(delegate.withTimeout(timeout, unit))
    }
...
private class UserLocatorBridge(
    private val instance: TeamCityInstanceBridge,
...
private class BuildBridge(private val delegate: org.jetbrains.teamcity.rest.coroutines.Build) : Build {
    ...
    override fun downloadArtifacts(pattern: String, outputDir: File) = runBlocking {
        delegate.downloadArtifact(pattern, outputDir)
    }
